...
export default class RawMaterial {
  ...
  insertRawMaterial = (req, res) => {
    ...
    if (!form_data || !form_data.name || !form_data.date) {
      return res
        .status(400)
        .json({ message: "form_data with name and date is required" });
    }

    if (!Array.isArray(raw_materials) || raw_materials.length === 0) {
      return res
        .status(400)
        .json({ message: "raw_materials must be a non-empty array" });
    }
    ...
    try {
      const saveBatch = db.transaction(() => {
        ...
        const product = db
          .prepare(
            "SELECT id FROM products WHERE name = ? AND type = ? LIMIT 1",
          )
          .get(name, type);
        ...
        const productRowForHistory = db
          .prepare(
            "SELECT barcode FROM products WHERE name = ? AND type = ? LIMIT 1",
          )
          .get(name, type);
        const barcodeForHistory = productRowForHistory
          ? productRowForHistory.barcode
          : null;

        const insertHistoryWithBarcode = db.prepare(`
                    INSERT INTO products_history (name, cost_price, stock, date, type, action, barcode)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `);
        insertHistoryWithBarcode.run(
          name,
          cost_price,
          stock,
          date,
          type,
          form_data.action || "Cost Calculation",
          barcodeForHistory,
        );
      ...
      if (String(err.message).includes("FOREIGN KEY")) {
        return res
          .status(400)
          .json({ message: "Invalid raw_id or product reference" });
      }
      if (String(err.message).includes("UNIQUE")) {
        return res
          .status(400)
          .json({ message: "Product must be unique per type" });
      }
      res.status(500).json({ message: "Internal Server Error" });
    }
  };
  ...
  updateRawMaterial = (req, res) => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: "raw_id, product_id, quantity and date are required",
        });
    }
    ...
    try {
      ...
      if (String(err.message).includes("FOREIGN KEY")) {
        return res
          .status(400)
          .json({ message: "Invalid raw_id or product_id" });
      }
      res.status(500).json({ message: "Internal Server Error" });
    }
  };
