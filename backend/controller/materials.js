...
export default class Material {
  ...
  getDailyMaterials = (req, res) => {
    ...
    try {
      const rows = db
        .prepare(
          "SELECT sum(amount) as total FROM materials WHERE date = ? group by date"
        )
        .all(date);
      res.json(rows);
    } catch (err) {
  ...
  insertMaterial = (req, res) => {
    ...
    try {
      const stmt = db.prepare(
        "INSERT INTO materials (title, description, amount, date) VALUES (?, ?, ?, ?)"
      );
  ...
  updateMaterial = (req, res) => {
    ...
    try {
      const stmt = db.prepare(
        "UPDATE materials SET title = ?, description = ?, amount = ?, date = ? WHERE id = ?"
      );
