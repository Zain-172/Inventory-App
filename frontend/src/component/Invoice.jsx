...
const Invoice = forwardRef(({ saleData }, ref) => {
  ...
  return (
    <>
      <div
        ref={ref}
        className="relative p-4 bg-white text-black w-[550px] min-h-80 print:bg-white print:text-black print:h-full print:w-full print:absolute print:top-0 print:left-0 print:right-0"
      >
        ...
        <div className="grid grid-cols-2 justify-between w-full">
          <p>
            <strong>Invoice:</strong> {saleData.invoice_id}
          </p>
          <p className="text-right">
            <strong>Date:</strong> {saleData.sale_date}
          </p>
          <p>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="grid grid-cols-[1fr_8fr_2fr_2fr_3fr]">
              <th className="border border-black bg-white text-black px-2 py-1">
                Sr.
              </th>
              <th className="border border-black bg-white text-black px-2 py-1">
                Items Description
          ...
          <tbody>
            {saleData.items.map((item, idx) => (
              <tr key={idx} className="grid grid-cols-[1fr_8fr_2fr_2fr_3fr]">
                <td className="border border-black text-black px-2 py-1">
                  {idx + 1}
                </td>
                <td className="border border-black text-black px-2 py-1">
                  {item.product_name}
      ...
      <button
        onClick={handleClick}
        className="bg-blue-900 rounded-lg py-1 font-bold grid place-self-center w-[550px] mt-6 no-print"
      >
        Print
      </button>
    </>
