...
export default function RawForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      ...
      setMaterials((prevMaterials) => [...prevMaterials, { id: data.Id, title: form.title, description: form.description, amount: form.amount, date: form.date }]);
      setForm(new Expense());
      alertBox("The Material is recorded successfully", "Success", <FaCheckCircle />);
    } else {
  ...
  return (
    ...
    >
      <h2 className="text-xl font-bold mb-4 text-center"><FaWarehouse /> Material</h2>

      <div className="grid grid-cols-2 gap-4 mb-4">
        <label className="text-sm">Title
          <input
            type="text"
            className="w-full p-2 bg-[#222] border rounded-lg mb-3"
            value={form.title}
      ...
      <input
        type="date"
        className="w-full p-2 bg-[#222] border rounded-lg mb-4"
        value={form.date}
      ...
      <textarea
        className="w-full p-2 bg-[#222] border border-white/20 resize-none rounded-lg mb-3"
