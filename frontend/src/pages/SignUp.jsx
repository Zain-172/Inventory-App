...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    const opt = await generateOTP(userData.email);
    setOTP(otp)
    setData(userData);
    setOtpOpen(true);
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black bg-opacity-20">
      <main className="bg-white rounded-lg shadow-lg w-full max-w-4xl grid grid-cols-1 md:grid-cols-[0.1fr_1fr_3fr]">
      ...
      <Modal isOpen={otpOpen} onClose={() => setOtpOpen(false)}>
        <OTP onSuccess={signUpUser} OTP={OTP} />
      </Modal>
    </div>
  );
}
